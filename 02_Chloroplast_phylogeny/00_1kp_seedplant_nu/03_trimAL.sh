...
#This array include all 78 genes
...
mkdir trimed renamed

for gene in "${array[@]}"

  do 
    trimal -in ../02_mafft-linsi/${gene}.fasta.mafft -out trimed/${gene}.fasta -gappyout -fasta

    #rename
    sed 's/ .*$//g' trimed/${gene}.fasta > renamed/${gene}.fasta
  done   
